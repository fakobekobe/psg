...
class TraitementEntraineur extends TraitementAbstrait
{
    ...
    {        
        ...
        foreach($donnees[0] as $data)
        {
            ...
            $photo = $data->getPhoto() ? Utilitaire::afficher_image(id: $data->getId(), url: "entraineur", path: $data->getPhoto()) : '';
    ...
    protected function actionAjouterSucces(mixed ...$donnees): JsonResponse
    {
        ...
        if ($donnees['donnees'][1]['image']) {
            ...
            $data->setPhoto(photo: $nomDuFichier);
        }       
    ...
    public function actionModifierSucces(mixed $objet = null): JsonResponse
    {
        ...
        if ($objet['image']) {
            ...
            $path = Utilitaire::getPathImage() . DIRECTORY_SEPARATOR . $data->getPhoto();
            Utilitaire::supprime_image(path: $path);
            $data->setPhoto($nomDuFichier);
        }
    ...
    {
        $path = Utilitaire::getPathImage() . DIRECTORY_SEPARATOR . $objet->getPhoto();
        Utilitaire::supprime_image(path: $path);

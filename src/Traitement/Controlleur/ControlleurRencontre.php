...
class ControlleurRencontre extends ControlleurAbstrait
{
    public function ajouter(mixed ...$donnees): array
    {
        ...
        if ($form->isSubmitted()) {
            ...
            if ($donnees[6] && count(value: $erreur) <= 1 && $erreur[$donnees[6]['libelle']] && $donnees[6]['objet']) {
                $estValide = true;
    ...
    public function modifier(mixed ...$donnees): JsonResponse
    {
        ...
        if ($donnees[7]) {
            $objet->setCalendrier($donnees[7]['objet']);
        }
        ...
        if ($form->isSubmitted()) {
            $erreur = Utilitaire::getErreur(form: $form);
            // Gestion d'un seul select
            if ($donnees[7] && count(value: $erreur) <= 1 && $erreur[$donnees[7]['libelle']] && $donnees[7]['objet']) {
                $estValide = true;
            }
        }
        ...
        return ($donnees[0])->getTraitement()->actionModifier($estValide, $objet);
    }
}

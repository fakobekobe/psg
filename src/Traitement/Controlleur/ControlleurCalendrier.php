...
class ControlleurCalendrier extends ControlleurAbstrait
{
    public function ajouter(mixed ...$donnees): array
    {
        ...
        if ($form->isSubmitted()) {
            $estValide = false;
            $journees = ($donnees[4])->request->all()['calendrier']['journee'] ?? null;
    ...
    public function modifier(mixed ...$donnees): JsonResponse
    {
        ...
        $estValide = false;
        $journees = ($donnees[5])->request->all()['calendrier']['journee'] ?? null;
        
        if ($journees) {
            $objet->setJournee(($donnees[0])->getJournee($journees[0]));
        ...
        return ($donnees[0])->getTraitement()->actionModifier($objet, $estValide);
    }
}
